...
@section('content')
    ...
    <div class="row card">
        <div class="card-body">
            <form action="{{ route('entry_harian.update', $entry->uuid) }}" method="POST" class="row g-3">
                @csrf
                @method('PUT')
                <div class="col-md-6 mb-2">
                    ...
                    <select id="rekening_id" class="form-select select2" name="rekening_id" required>
                        ...
                        @foreach ($rekenings as $rek)
                            <option value="{{ $rek->id }}" {{ $entry->rekening_id == $rek->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6 mb-2">
                    ...
                    <select id="via_id" class="form-select select2" name="via_id" required>
                        ...
                        @foreach ($vias as $via)
                            <option value="{{ $via->id }}" {{ $entry->via_id == $via->id ? 'selected' : '' }}>
                                {{ $via->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-2">
                    ...
                    <input type="date" class="form-control" id="tanggal_setor" name="tanggal_setor"
                        value="{{ $entry->tanggal_setor }}" required>
                </div>
                <div class="col-md-6 mb-2">
                    ...
                    <input type="number" class="form-control" id="jumlah_bayar" name="jumlah_bayar"
                        value="{{ $entry->jumlah_bayar }}" required>
                </div>
